...
function WorkshopDetailsModal({
  ...
  useEffect(() => {
    const fetchDataUrl = async () => {
      try{
        const data = await readDocument(COLLECTIONS.WORKSHOPS,dataItemId)
        setDataItem(data)
...
export default function NWorkshopCard({dataItem, studioIdName}) {
  const isDarkModeOn = useSelector(selectDarkModeStatus);
    ...
    <>
      ...
      >
        ...
        <CardContent style={{ padding: "10px", paddingTop: "5px" }}>
          ...
          >
            <span>
              {dataItem && dataItem.workshopName ? dataItem.workshopName : "0"}
            </span>
          </Box>
          <Typography
