...
function MyBookings() {
    ...
    const fetchData = async () => {
      try {
        ...
        const query_ref = query(
          collection(db, COLLECTIONS.FREE_TRIAL_BOOKINGS),
              ...
              >
                <p style={{ fontSize: "small" }}>
                  ...
                  <span style={{ fontSize: "small" }}>
                    {new Date(bookingData.timestamp * 1000).toLocaleDateString(
                      "en-US",
                      {
                        month: "short",
                        year: "numeric",
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td style={{ padding: "8px 0", textAlign: "right" }}>
                        ₹{parseInt(bookingData.price_per_person)}
                      </td>
                    </tr>
                    <tr>
                      <td style={{ padding: "8px 0" }}>
                      ...
                      <td style={{ padding: "8px 0", textAlign: "right" }}>
                        {parseInt(bookingData.persons_allowed)}
                      </td>
                    </tr>
                    <tr>
                      <td
  ...
  const bookingCategories = {
    Workshops: userBookings?.data?.Workshops
      ? Object.entries(userBookings.data.Workshops).map(([key, value]) => ({
          ...value,
          id: key,
        }))
      : [],
    OpenClasses: userBookings?.data?.OpenClasses
      ? Object.entries(userBookings.data.OpenClasses).map(([key, value]) => ({
          ...value,
          id: key,
        }))
      : [],
    Courses: userBookings?.data?.Courses
      ? Object.entries(userBookings.data.Courses).map(([key, value]) => ({
          ...value,
          id: key,
        }))
      : [],
    ...
    <div style={{ backgroundColor: isDarkModeOn ? "#202020" : "white" }}>
      ...
      <Box>
        ...
        {tabIndex === 0 && (
          <Box py={3}>
            <div
              style={{ backgroundColor: isDarkModeOn ? "#202020" : "white" }}
            >
              {bookings.length === 0 ? (
                <div>
                ...
                <div>
                  {currentClickTicket ? (
                    <BookingInformation
                      currentClickTicket={currentClickTicket}
                      setCurrentClickTicket={setCurrentClickTicket}
                    />
                  ) : (
                    <>
        ...
        {tabIndex === 2 && (
          <Box p={3}>
            ...
            {openClassClickTicket ? (
              <OpenClassInformation
                openClassClickTicket={openClassClickTicket}
                setOpenClassClickTicket={setOpenClassClickTicket}
              />
            ) : (
              <>
